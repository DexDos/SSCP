...
>
>A `docker-compose.yml` is a file which is used by [[Docker|Docker CLI]] to deploy versatile containers. We may start this container with both `docker-compose` (outdated, written in python) and `docker compose` (faster, more reliable, not a separate util, but embedded into Docker CLI, which is written in GoLang).

The file needs no be composed in a certain syntax of `.yml` and contain specific keys:
![[Pasted image 20260730172035.png]]
...
![[Pasted image 20260730173958.png]]
...
> 
> More about [[netdiscover]]

The output of the command:
![[Pasted image 20260730173827.png|656]]
...
![[Pasted image 20260730173432.png]]
...
![[Pasted image 20260801152312.png]]
## Registration
We also need to register:
![[Pasted image 20260801152505.png]]

Fill out the registration form:
![[Pasted image 20260801152542.png]]

We may now proceed to the assignments:
![[Pasted image 20260801152609.png]]
...
![[Pasted image 20260801154037.png]]

Proxy configuration:
![[Pasted image 20260801162147.png]]
...
>![[Pasted image 20260801160219.png]]
...
>![[Pasted image 20260801161248.png]]
...
>The proxy itself is a technology that redirects traffic. This technology can be used to send HTTP Connect method, thus redirecting traffic completely via proxy server. However, in this case we simply redirecting in and outbound traffic to the port specified.

By default, Burp Suite listens on the 8080 port. Unfortunately, the WebGoat server listens on the exact same port. To evade any kind of conflict we have specified `8123` port for FoxyProxy configuration. 
### Burp Suite
We have picked the **Burp Suite** to be the program which will be intercepting our http traffic. However there are various programs to use instead. For example **ZAP**.

Let's configure the Burp Proxy:
![[Pasted image 20260801162634.png]]

Then we need to specify the socket which we will be listening: `loopback:8123`:
![[Pasted image 20260801162812.png]]

Afterwards we can turn the proxy profile on and test our proxy:
![[Pasted image 20260801163203.png]]
...
![[Pasted image 20260801173207.png]]

By doing so we now may see the traffic by requesting `http://localhost.com:8080/WebGoat/login` page:
![[Pasted image 20260801173134.png]]

Alternatively we can simply use Burp built-in browser:
![[Pasted image 20260801173327.png]]

The result is pretty much the same:
![[Pasted image 20260801173302.png]]
## Cases
There are two introductory sections: *introduction* and *general*.
- **Introduction** lists some basic facts about WebGoat and WebWolf further usage.
- **General** dives a bit into the networking, dev tools and other stuff.
...
| ------------------------------------------------------------------- | --------------------- | ----------- |
| [[IT/CyberSec/Targets/OWASP WebGoat/1\|Broken Access Control]]      | **done \| uploaded**  |             |
| [[IT/CyberSec/Targets/OWASP WebGoat/2\|Cryptographic failures]]     | done \| not reachable |             |
| [[IT/CyberSec/Targets/OWASP WebGoat/3\|Injection]]                  | done \| not reachable |             |
| [[IT/CyberSec/Targets/OWASP WebGoat/4\|Security Misconfiguration]]  | done \| not reachable |             |
| [[IT/CyberSec/Targets/OWASP WebGoat/5\|Vuln & Outdated Components]] | to be done            |             |
| [[IT/CyberSec/Targets/OWASP WebGoat/6\|Identity & Auth Failure]]    | to be done            |             |
| [[7\|Software & Data Integrity]]                                    | to be done            |             |
| [[8\|Security & Login Failures]]                                    | to be done            |             |
| [[9\|Server-side Request Forgery]]                                  | to be done            |             |
## Flags
