...
Technology related to: "[IDS_IPS](IDS_IPS.md)"
Source:
---
## Overview
...
More information is listed in the [IDS/IPS technology note](IDS_IPS.md).
### Key features
...
![Pasted image 20260803100137.png](../../../Cache/IMGs/Pasted%20image%2020260803100137.png)
...
1. Define the network **IP range**:
   ![Pasted image 20260803101032.png](../../../Cache/IMGs/Pasted%20image%2020260803101032.png)
   
2. Update the `HOME_NET` accordingly:![Pasted image 20260803100610.png](../../../Cache/IMGs/Pasted%20image%2020260803100610.png)
   
3. Make sure the `interface` variable in `af-packet` matches the interface used:
   ![Pasted image 20260803101217.png](../../../Cache/IMGs/Pasted%20image%2020260803101217.png)
...
4. `cluster-id:99` – assigns a cluster identifier to the packets which is useful for categorizing and monitoring packets flow in order to determine which packets belong to which cluster
   ![Pasted image 20260803101421.png](../../../Cache/IMGs/Pasted%20image%2020260803101421.png)
...
5. `cluster-type: cluster_flow` – specifies the type of clustering that will be applied to the packets. In this case, it is `cluster_flow`, which indicates that the clustering is based on flow.
   ![Pasted image 20260803101517.png](../../../Cache/IMGs/Pasted%20image%2020260803101517.png)
   
6. `defrag: yes` – enables packet **defragmentation**:
   ![Pasted image 20260803101604.png](../../../Cache/IMGs/Pasted%20image%2020260803101604.png)
...
7. `tpacket-v3: yes` – specifies the use of `TPACKET_V3`, which is a packet capture method on Linux systems. `TPACKET_V3` offers improved performance and efficiency compared to previous versions.
   ![Pasted image 20260803102616.png](../../../Cache/IMGs/Pasted%20image%2020260803102616.png)
...
8. `use-mmap:yes` Enables the ring buffer mechanism (PACKET_MMAP) via pcap/socket. This allows network packets to be transferred from the kernel to user space directly through shared memory, minimizing unnecessary data copying and reducing the CPU load.
   ...
   **ACCORDING TO AI:** In modern versions of Suricata, the ring buffer is already used by default for the `af-packet` driver at the Linux kernel level, so explicitly specifying this directive within the `af-packet` block is most often a legacy requirement from older guides. 
   ...
   ![Pasted image 20260803102736.png](../../../Cache/IMGs/Pasted%20image%2020260803102736.png)
...
![Pasted image 20260803104221.png](../../../Cache/IMGs/Pasted%20image%2020260803104221.png)

#### Logs inspection
A section primarily about log files
##### Functioning
We can inspect Suricata functionality logs: `sudo tail /var/log/suricata/suricata.log`
![Pasted image 20260803104335.png](../../../Cache/IMGs/Pasted%20image%2020260803104335.png)

##### Performance
We can also inspect performance metrics and counters: `sudo tail -f /var/log/suricata/stats.log`
![Pasted image 20260803104430.png](../../../Cache/IMGs/Pasted%20image%2020260803104430.png)
...
![Pasted image 20260803104630.png](../../../Cache/IMGs/Pasted%20image%2020260803104630.png)

#### Logs testing
We can test one of the security features of Suricata by running`curl http://testmynids.org/uid/index.html`
![Pasted image 20260803104709.png](../../../Cache/IMGs/Pasted%20image%2020260803104709.png)
...
![Pasted image 20260803120207.png](../../../Cache/IMGs/Pasted%20image%2020260803120207.png)

#### JQ
We can utilize [JSON](JSON.md) parser util –  [jq](jq.md) in order to dynamically select properly categorized logs among the whole bunch of them: `$ sudo tail -f /var/log/suricata/eve.json| jq 'select(.event_type=="stats")'`

![Pasted image 20260803105136.png](../../../Cache/IMGs/Pasted%20image%2020260803105136.png)
### Suricata rules
...
![Pasted image 20260803111705.png](../../../Cache/IMGs/Pasted%20image%2020260803111705.png)
...
![Pasted image 20260803112207.png](../../../Cache/IMGs/Pasted%20image%2020260803112207.png)
## Assignments
...
### Starting Suricata
Is listed within the [Running Suricata](#running-suricata) section.
...
- Outbound query containing the key word _"ATTACK"_ – to show that tracking query content is possible
...
- Running it as a process gives us an opportunity to quickly inspect it status. It also _captures_ the shell, so we will need a separate one for testing.
    
- Running is as a daemon would be performed in a background. We will need to use `systemctl` and `journalctl` to access it direct output. Eventually, both approaches are applicable, so feel free to use the more convenient one.
